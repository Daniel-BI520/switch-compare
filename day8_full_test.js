...
console.log("交换机智能匹配 - 全量73案例测试（Day 1-8）");
console.log("=".repeat(70));
console.log(`总案例：${allCases.length}个（含Day8新增8个）`);
console.log(`数据库：${allSwitches.length}款交换机`);
...
for (const tier of ['核心', '汇聚', '接入']) {
    ...
    console.log(`  ${tier}层: ${tierResults.length}个有效, Top1=${top1}(${tierResults.length?Math.round(top1/tierResults.length*100):0}%), Top3=${top3}(${tierResults.length?Math.round(top3/tierResults.length*100):0}%), Top5=${top5}(${tierResults.length?Math.round(top5/tierResults.length*100):0}%)`);
}
...
for (const vendor of ['华为', 'H3C', '锐捷']) {
    ...
    console.log(`  ${vendor}: ${vResults.length}个有效, Top1=${top1}(${vResults.length?Math.round(top1/vResults.length*100):0}%), Top3=${top3}(${vResults.length?Math.round(top3/vResults.length*100):0}%), Top5=${top5}(${vResults.length?Math.round(top5/vResults.length*100):0}%)`);
}

// 总体结果
console.log("\n" + "=".repeat(70));
console.log(`总体命中率（有效案例 ${validCases}/${allCases.length}）:`);
...
missed.forEach((r, i) => {
    console.log(`  ${i+1}. ${r.project} - 第${r.rank}名 (${r.vendor}/${r.tier})`);
});

// Day 7 新增案例分析
console.log("\n" + "=".repeat(70));
console.log("Day 7 新增案例分析（7个案例）：");
console.log("=".repeat(70));
const day7Cases = results.slice(58, 65);
day7Cases.forEach((r, i) => {
    const status = r.top1 ? '✅Top1' : (r.top3 ? '✅Top3' : (r.top5 ? '✅Top5' : '❌'));
    console.log(`  ${i+1}. ${r.project} → 第${r.rank}名 [${r.vendor}/${r.tier}] ${status}`);
});

// Day 8 新增案例分析
console.log("\n" + "=".repeat(70));
console.log("Day 8 新增案例分析（8个新案例）：");
console.log("=".repeat(70));
const day8Cases = results.slice(65);
day8Cases.forEach((r, i) => {
    const status = r.top1 ? '✅Top1' : (r.top3 ? '✅Top3' : (r.top5 ? '✅Top5' : '❌'));
    console.log(`  ${i+1}. ${r.project} → 第${r.rank}名 [${r.vendor}/${r.tier}] ${status}`);
});
...
// 与Day6同测试集对比（前58个案例）
console.log("\n" + "=".repeat(70));
console.log("与Day6同测试集对比（前58案例）：");
console.log("=".repeat(70));
const day1to6Results = results.slice(0, 58);
const d6_valid = day1to6Results.filter(r => r.rank !== '未找到').length;
const d6_top1 = day1to6Results.filter(r => r.top1).length;
const d6_top3 = day1to6Results.filter(r => r.top3).length;
const d6_top5 = day1to6Results.filter(r => r.top5).length;
console.log(`  Top1: ${d6_top1}/${d6_valid} (${Math.round(d6_top1/d6_valid*100)}%) [Day6基线: 15%]`);
console.log(`  Top3: ${d6_top3}/${d6_valid} (${Math.round(d6_top3/d6_valid*100)}%) [Day6基线: 35%]`);
console.log(`  Top5: ${d6_top5}/${d6_valid} (${Math.round(d6_top5/d6_valid*100)}%) [Day6基线: 42%]`);
