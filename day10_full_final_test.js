...
console.log("交换机智能匹配算法 Day 10 全量测试报告");
console.log("=".repeat(80));
console.log(`总案例：${allCases.length}个（Day1-8 全部案例）`);
...
for (const tier of ['核心', '汇聚', '接入']) {
    ...
    console.log(`  ${tier}层: ${total}个案例 | Top1=${top1}(${Math.round(top1/total*100)}%) Top3=${top3}(${Math.round(top3/total*100)}%) Top5=${top5}(${Math.round(top5/total*100)}%)`);
}
...
for (const vendor of ['华为', 'H3C', '锐捷']) {
    ...
    console.log(`  ${vendor}: ${total}个案例 | Top1=${top1}(${Math.round(top1/total*100)}%) Top3=${top3}(${Math.round(top3/total*100)}%) Top5=${top5}(${Math.round(top5/total*100)}%)`);
}

// 按行业（简单分类）
console.log("\n【按行业分布（Top5命中率）】");
const industries = [
    { name: '政府/税务/公安', pattern: /税务|公安|海关|财政部|人行|征信/ },
    { name: '教育/医疗', pattern: /大学|学院|中学|小学|医院|医药|疾控/ },
    { name: '企业/金融', pattern: /银行|亿达|广电|信/ },
];
for (const ind of industries) {
    const indResults = results.filter(r => ind.pattern.test(r.project) && r.rank !== '未找到');
    const top5 = indResults.filter(r => r.top5).length;
    const total = indResults.length;
    if (total > 0) console.log(`  ${ind.name}: ${total}个 | Top5=${top5}(${Math.round(top5/total*100)}%)`);
}
...
// 未找到的型号
console.log("\n【数据库中不存在的中标型号】");
const notFound = results.filter(r => r.rank === '未找到');
notFound.forEach((r, i) => {
    console.log(`  ${i+1}. ${r.project} - ${r.winner} (${r.vendor})`);
});
